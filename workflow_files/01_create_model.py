...
- base_uri: str        (e.g., "https://app.globalmoo.com/api/")

OUTPUT VARIABLES (available after running):
- model_id: int        (the created model's ID - pass to next step)
...
credentials = create_credentials(
    api_key=api_key,      # INPUT: defined in interface
    base_uri=base_uri     # INPUT: defined in interface
)
...
model_req = create_model_request(
    "Optimization Model",
    "Model created for inverse optimization workflow"
)
...
if is_error(model):
    ...
    close_client(client)
    raise Exception(f"Model creation error: {model['message']}")
...
# Close client (we'll create a new one in the next step)
close_client(client)
