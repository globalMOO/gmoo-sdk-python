...
- model_id: int             (from Step 1)
- api_key: str              (from Step 1)
- base_uri: str             (from Step 1)
- input_count: int          (number of inputs, e.g., 2)
- input_min: List[float]    (minimum values for each input, e.g., [0.0, 0.0])
- input_max: List[float]    (maximum values for each input, e.g., [10.0, 10.0])

OUTPUT VARIABLES (available after running):
- project_id: int           (the created project's ID - pass to Step 4)
- input_cases: List[List[float]]  (input cases to evaluate - pass to Step 3)
"""
...
# Build input_types list (all floats for now)
input_types = [INPUT_TYPE_FLOAT] * input_count

project_req = create_project_request(
    model_id=model_id,              # INPUT: from Step 1
    name="Optimization Project",
    input_count=input_count,        # INPUT: from interface
    minimums=input_min,             # INPUT: from interface
    maximums=input_max,             # INPUT: from interface
...
if is_error(project_req):
    ...
    close_client(client)
    raise Exception(f"Project request error: {project_req['message']}")
...
if is_error(project):
    ...
    close_client(client)
    raise Exception(f"Project creation error: {project['message']}")

# OUTPUT: These variables will be available for the next step
project_id = project['id']
input_cases = project['inputCases']

print(f"✓ Created project with ID: {project_id}")
print(f"✓ Generated {len(input_cases)} input cases to evaluate")
print(f"✓ First few cases: {input_cases[:min(3, len(input_cases))]}")

# Close client
close_client(client)

# OUTPUTS for next step:
# - project_id (pass to Step 4)
# - input_cases (pass to Step 3 for function evaluation)
