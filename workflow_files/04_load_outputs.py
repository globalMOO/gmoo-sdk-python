"""
STEP 4: Load Output Cases
==========================

INPUT VARIABLES (define in interface):
- project_id: int                  (from Step 2)
- output_cases: List[List[float]]  (from Step 3)
- output_count: int                (from Step 3)
- api_key: str                     (from Step 1)
- base_uri: str                    (from Step 1)

OUTPUT VARIABLES (available after running):
- trial_id: int                    (the created trial's ID - pass to Step 5)
"""
...
# Step 3: Load output cases
trial_req = load_output_cases_request(
    project_id=project_id,      # INPUT: from Step 2
    output_count=output_count,  # INPUT: from Step 3
    output_cases=output_cases   # INPUT: from Step 3
)

if is_error(trial_req):
    print_error_message(trial_req)
    close_client(client)
    raise Exception(f"Trial request error: {trial_req['message']}")
...
if is_error(trial):
    ...
    close_client(client)
    raise Exception(f"Trial creation error: {trial['message']}")

# OUTPUT: This variable will be available for the next step
trial_id = trial['id']

print(f"✓ Created trial with ID: {trial_id}")
print(f"✓ Loaded {len(output_cases)} output cases")

# Close client
close_client(client)

# OUTPUTS for next step:
# - trial_id (pass to Step 5)
# - output_cases (pass to Step 5 - needed for initial values)
